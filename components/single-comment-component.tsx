...
export default function SingleCommentComponent({props} : any)  {
  ...
  console.log(props);
  useEffect(() => {
    fetch(`http://localhost:8000/comments/${props}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        setData(data)
  ...
  return (
    <>
        ...
        data.map((element:any) => {
         return <div className="flex flex-row border bg-white border-gray-300 p-4">
          <img src={((element.user == null)? "unknown-avatar.png": element.user.img)} alt="avatar" className=" cursor-pointer w-10 h-10 rounded-full ml-2" />
